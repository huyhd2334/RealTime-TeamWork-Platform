...
const MainCr = () => {
  ...
  const handleJoinRoom = async () => {
    ...
    setRoomCode(code);
    const isExistRoom = await api.post("/checkroom", {roomCode: roomCode})
    if (isExistRoom.data.message) {
      toast.success(isExistRoom.data.detail)
      navigate("/liveroom", { state: { roomCode: roomCode } });    
    } else {
      toast.error(isExistRoom.data.detail)
    }
  };
  return (
